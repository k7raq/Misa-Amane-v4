...
class AutoButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not c:
            await interaction.response.send_message("This challenge is no longer active.", ephemeral=True)
            return
        if interaction.user.id != c["opponent"]:
            await interaction.response.send_message("Only the challenged player can choose Auto.", ephemeral=True)
            return
...
async def finish_challenge(guild: discord.Guild, c: dict, winner: int, auto: bool, reason: str, score: Optional[str], referee: Optional[str], set_id: Optional[int]):
    ...
    if score_ch:
        ...
        ping_role = guild.get_role(SCORE_PING_ROLE_ID)
        score_content = ping_role.mention if ping_role else None
        msg = await score_ch.send(content=score_content, embed=e)
        if REVIEW_CHANNEL_ID:
            review_ch = await fetch_text_channel(guild, REVIEW_CHANNEL_ID)
            if review_ch:
                review_content = ping_role.mention if ping_role else None
                review_embed = e.copy()
                review_embed.title = "Premier League • Score Review"
                await review_ch.send(content=review_content, embed=review_embed)
...
@bot.tree.command(name="editspot", description="Edit a leaderboard spot's player and stage.")
@app_commands.choices(
    board=[
        app_commands.Choice(name="Overall", value="overall"),
        app_commands.Choice(name="Mobile", value="mobile")
    ],
    region=[app_commands.Choice(name=x, value=x) for x in REGIONS]
)
async def editspot(
    ...
    target_uid = None
    for key, p in DATA["profiles"].items():
        if int(p.get("profile_id", 0)) == player_id:
            profile = p
            target_uid = int(key)
            break

    if not profile or target_uid is None:
        await interaction.response.send_message(
            f"Player ID **{player_id}** was not found. Use `/scan` to find the correct player ID.",
            ephemeral=True
        )
        return
    ...
    # Remove the player from any old position in this exact LB before placing them.
    old_spot = find_player(board, region, target_uid)
    if old_spot is not None and old_spot != spot:
        spots[old_spot - 1] = None

    # If another player already occupies the target, editor replacement is intentional.
    spots[spot - 1] = {"user_id": target_uid, "stage": stage.strip()}

    save()
    await refresh_lb(board, region, interaction.guild)
    await interaction.response.send_message(
        f"Updated **{board.title()} • {region} #{spot}** to "
        f"**{profile.get('nickname', 'Unknown')}** (Player ID `{player_id}`) "
        f"with stage **{stage.strip() or '—'}**.",
        ephemeral=True
    )


@bot.tree.command(name="scan", description="Find a player's stored profile ID using their Roblox username.")
async def scan(interaction: discord.Interaction, player_username: str):
    if not is_editor(interaction.user):
        await interaction.response.send_message("Editor role required.", ephemeral=True)
        return

    query = player_username.strip().lower()
    matches = []
    for p in DATA["profiles"].values():
        if str(p.get("roblox_username", "")).lower() == query:
            matches.append(p)

    if not matches:
        await interaction.response.send_message(
            f"No stored LB profile was found for Roblox username **{player_username}**.",
            ephemeral=True
        )
        return

    for p in matches[:10]:
        uid = p.get("discord_id")
        e = discord.Embed(
            title=f"Player Scan • ID #{int(p.get('profile_id', 0)):03d}",
            description=f"**{p.get('nickname', 'Unknown')}**\nDiscord: <@{uid}>"
        )
        e.add_field(name="Roblox", value=p.get("roblox_username", "—"), inline=True)
        e.add_field(name="Country", value=p.get("country_flag", "—"), inline=True)
        e.add_field(name="Profile ID", value=f"`{int(p.get('profile_id', 0))}`", inline=True)
        if p.get("avatar_url"):
